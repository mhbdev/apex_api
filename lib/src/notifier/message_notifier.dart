...
class MessageNotifier<T extends dynamic> {
  ...
  // performance.
  static final List<MessageNotifierCallback?> _emptyListeners =
  List<MessageNotifierCallback?>.filled(0, null);
  List<MessageNotifierCallback?> _listeners = _emptyListeners;
  ...
  /// [MessageNotifier] not being able to determine which listener is being
  ...
  void addListener(MessageNotifierCallback listener) {
    assert(_debugAssertNotDisposed());
    if (_count == _listeners.length) {
      if (_count == 0) {
        _listeners = List<MessageNotifierCallback?>.filled(1, null);
      } else {
        final List<MessageNotifierCallback?> newListeners =
        List<MessageNotifierCallback?>.filled(_listeners.length * 2, null);
  ...
  void _removeAt(int index) {
    ...
    if (_count * 2 <= _listeners.length) {
      final List<MessageNotifierCallback?> newListeners =
      List<MessageNotifierCallback?>.filled(_count, null);
  ...
  ///    changes.
  void removeListener(MessageNotifierCallback listener) {
    ...
    for (int i = 0; i < _count; i++) {
      final MessageNotifierCallback? listenerAtIndex = _listeners[i];
  ...
  void notifyListeners(dynamic tag, [T? message]) {
      ...
      try {
        ...
        FlutterError.reportError(FlutterErrorDetails(
          ...
          informationCollector: () => <DiagnosticsNode>[
            DiagnosticsProperty<MessageNotifier>(
    ...
    if (_notificationCallStackDepth == 0 && _reentrantlyRemovedListeners > 0) {
      ...
      if (newLength * 2 <= _listeners.length) {
        ...
        final List<MessageNotifierCallback?> newListeners =
        List<MessageNotifierCallback?>.filled(newLength, null);

        int newIndex = 0;
        for (int i = 0; i < _count; i++) {
          final MessageNotifierCallback? listener = _listeners[i];
          if (listener != null) {
